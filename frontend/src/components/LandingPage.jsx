...
const G = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .modal-backdrop { position: fixed; inset: 0; background: rgba(3,5,14,0.88); backdrop-filter: blur(12px); z-index: 200; display: flex; align-items: center; justify-content: center; padding: 20px; }
  .modal-card { background: #0a0f1e; border: 1px solid #1a2540; border-radius: 24px; padding: 44px 40px; width: 100%; max-width: 420px; box-shadow: 0 40px 80px rgba(0,0,0,0.7); animation: modalIn 0.4s cubic-bezier(0.16,1,0.3,1) both; }
  .auth-input { width: 100%; background: rgba(255,255,255,0.04); border: 1px solid #1a2540; border-radius: 10px; padding: 14px 16px; color: #ffffff; font-family: 'Geist', sans-serif; font-size: 15px; outline: none; transition: border-color 0.2s; }
  ...
  .primary-btn { width: 100%; padding: 14px; background: #ffffff; border: none; border-radius: 10px; color: #050810; font-family: 'Geist', sans-serif; font-size: 14px; font-weight: 600; cursor: pointer; transition: opacity 0.2s, transform 0.15s; }
  .primary-btn:hover { opacity: 0.9; transform: translateY(-1px); }
  .primary-btn:disabled { opacity: 0.4; cursor: not-allowed; transform: none; }
  .divider { display: flex; align-items: center; gap: 12px; color: #1a2540; font-size: 12px; }
  .divider::before, .divider::after { content: ''; flex: 1; height: 1px; background: #1a2540; }

  /* Landing specific */
  .lp-nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; padding: 0 clamp(20px,5vw,80px); height: 64px; display: flex; align-items: center; justify-content: space-between; background: rgba(5,8,16,0.7); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(255,255,255,0.04); }
  ...
  .lp-grid-3 { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1px; background: #0d1428; border: 1px solid #0d1428; border-radius: 20px; overflow: hidden; }
  .lp-card { background: #050810; padding: clamp(24px,3vw,40px); transition: background 0.3s; }
  .lp-card:hover { background: #080d1c; }
  ...
  .tag { display: inline-flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); border-radius: 20px; padding: 5px 14px; font-size: 12px; color: rgba(255,255,255,0.5); }
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target.classList.contains("modal-backdrop") && onClose()}>
      <div className="modal-card">
        ...
        <div style={{ display: "flex", gap: 4, background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: 4, marginBottom: 20, border: "1px solid #1a2540" }}>
          {["login", "signup"].map(m => (
            <button key={m} onClick={() => { setMode(m); setError(""); }}
              style={{ flex: 1, padding: "9px", border: "none", borderRadius: 7, background: mode === m ? "rgba(255,255,255,0.08)" : "transparent", color: mode === m ? "#ffffff" : "rgba(255,255,255,0.35)", fontFamily: "'Geist', sans-serif", fontSize: 13, fontWeight: 500, cursor: "pointer", transition: "all 0.2s" }}>
        ...
        <p style={{ textAlign: "center", marginTop: 20, fontSize: 12, color: "rgba(255,255,255,0.2)" }}>
          ...
          <span onClick={() => { setMode(mode === "login" ? "signup" : "login"); setError(""); }} style={{ color: "rgba(255,255,255,0.5)", cursor: "pointer", textDecoration: "underline" }}>
    ...
    <>
      ...
      <nav className="lp-nav">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 9, background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.5)", padding: "2px 7px", borderRadius: 4, letterSpacing: "0.1em", fontWeight: 500 }}>INDIA</span>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <a href="#about" style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", textDecoration: "none", padding: "6px 12px" }} className="hide-mobile">About</a>
          <a href="#features" style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", textDecoration: "none", padding: "6px 12px" }} className="hide-mobile">Features</a>
          {user ? (
            <button onClick={onLaunchApp} style={{ padding: "8px 20px", background: "#ffffff", color: "#050810", border: "none", borderRadius: 8, fontSize: 13, fontFamily: "'Geist', sans-serif", fontWeight: 600, cursor: "pointer" }}>
              Open Dashboard →
            </button>
          ) : (
            <button onClick={() => setShowAuth(true)} style={{ padding: "8px 20px", background: "#ffffff", color: "#050810", border: "none", borderRadius: 8, fontSize: 13, fontFamily: "'Geist', sans-serif", fontWeight: 600, cursor: "pointer" }}>
              Get Started
            </button>
          )}
        </div>
      ...
      <section className="lp-hero">
        ...
        {/* Glow */}
        <div style={{ position: "absolute", top: "20%", left: "50%", transform: "translateX(-50%)", width: 600, height: 600, background: "radial-gradient(circle, rgba(255,255,255,0.03) 0%, transparent 70%)", pointerEvents: "none" }} />

        <div style={{ position: "relative", zIndex: 1, maxWidth: 800, animation: "fadeUp 0.8s ease both" }}>
          ...
          <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: "clamp(40px, 7vw, 88px)", lineHeight: 1.02, letterSpacing: "-0.03em", color: "#ffffff", marginBottom: 24 }}>
            What will India<br />
            wear <span style={{ fontStyle: "italic", color: "rgba(255,255,255,0.35)" }}>tomorrow?</span>
          </h1>

          <p style={{ fontSize: "clamp(15px, 2vw, 19px)", color: "rgba(255,255,255,0.45)", lineHeight: 1.7, maxWidth: 520, margin: "0 auto 44px", fontWeight: 300 }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={user ? onLaunchApp : () => setShowAuth(true)}
              style={{ padding: "14px 32px", background: "#ffffff", color: "#050810", border: "none", borderRadius: 12, fontSize: 15, fontFamily: "'Geist', sans-serif", fontWeight: 700, cursor: "pointer", transition: "transform 0.15s, opacity 0.15s" }}
              onMouseEnter={e => e.currentTarget.style.transform = "translateY(-2px)"}
              onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}>
              {user ? "Open Dashboard →" : "Start Forecasting Free →"}
            </button>
            <a href="#features" style={{ padding: "14px 32px", background: "transparent", color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, fontSize: 15, fontFamily: "'Geist', sans-serif", fontWeight: 500, cursor: "pointer", textDecoration: "none", transition: "border-color 0.2s, color 0.2s" }}
        ...
        {/* Stats bar */}
        <div style={{ position: "relative", zIndex: 1, display: "flex", gap: 0, marginTop: 80, border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, overflow: "hidden", animation: "fadeUp 0.8s ease 0.2s both", flexWrap: "wrap" }}>
          {[
            ...
            <div key={i} style={{ padding: "28px 40px", background: "rgba(255,255,255,0.02)", borderRight: i < 3 ? "1px solid rgba(255,255,255,0.06)" : "none", textAlign: "center", flex: 1, minWidth: 120 }}>
              <div className="stat-num">{s.num}</div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", marginTop: 6, letterSpacing: "0.05em" }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="features" className="lp-section">
        <div style={{ marginBottom: 64, animation: "fadeUp 0.6s ease both" }}>
      ...
      {/* HOW IT WORKS */}
      <section style={{ padding: "clamp(60px,8vw,120px) clamp(20px,5vw,80px)", borderTop: "1px solid rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
          ...
          <div className="lp-steps">
            {[
              ...
              <div key={i} style={{ animation: `fadeUp 0.5s ease ${i * 0.1}s both` }}>
                <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: 48, color: "rgba(255,255,255,0.06)", lineHeight: 1, marginBottom: 20 }}>{s.num}</div>
      ...
      <section className="lp-section">
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          {[
            ...
            <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "14px 20px", animation: `fadeUp 0.4s ease ${i * 0.05}s both` }}>
              <div style={{ fontSize: 14, fontWeight: 500, color: "#ffffff", marginBottom: 3 }}>{s.name}</div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.25)" }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ABOUT */}
      <section id="about" style={{ padding: "clamp(60px,8vw,120px) clamp(20px,5vw,80px)", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
      ...
      {/* CTA */}
      <section style={{ padding: "clamp(60px,8vw,120px) clamp(20px,5vw,80px)", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Instrument Serif', serif", fontSize: "clamp(32px, 5vw, 64px)", letterSpacing: "-0.03em", color: "#ffffff", marginBottom: 20, lineHeight: 1.05 }}>
            Stop guessing.<br />
            <span style={{ fontStyle: "italic", color: "rgba(255,255,255,0.35)" }}>Start forecasting.</span>
          </h2>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.35)", lineHeight: 1.7, marginBottom: 40 }}>
            Free to get started. No credit card required.
          </p>
          <button onClick={user ? onLaunchApp : () => setShowAuth(true)}
            style={{ padding: "16px 40px", background: "#ffffff", color: "#050810", border: "none", borderRadius: 12, fontSize: 16, fontFamily: "'Geist', sans-serif", fontWeight: 700, cursor: "pointer", transition: "transform 0.15s" }}
            onMouseEnter={e => e.currentTarget.style.transform = "translateY(-2px)"}
            onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}>
      ...
      {/* FOOTER */}
      <footer style={{ padding: "32px clamp(20px,5vw,80px)", borderTop: "1px solid rgba(255,255,255,0.04)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontFamily: "'Instrument Serif', serif", fontSize: 16, color: "rgba(255,255,255,0.6)" }}>TrendCast India</span>
        </div>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.2)" }}>
          Powered by Claude AI · © 2026 TrendCast
        </div>
        <div style={{ display: "flex", gap: 20 }}>
          {["Features", "About", "Privacy"].map(l => (
            <a key={l} href={`#${l.toLowerCase()}`} style={{ fontSize: 12, color: "rgba(255,255,255,0.25)", textDecoration: "none" }}
